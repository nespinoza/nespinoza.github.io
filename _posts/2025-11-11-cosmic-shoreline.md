...
The key insight is that the right-hand side of this equation, except for $t$, are all planetary parameters at heart. The left-hand side is, in principle, all about the star for a planet at a fixed distance from it. Now, let's rewrite the right-hand side only in terms of the escape velocity, $v_{esc} = \sqrt{GM_p/R_p}$ and the planetary density, $\rho_p = (3/4 \pi ) M_p/R_p^3$. Noting that
...
$$ I \gtrsim \frac{f}{t \epsilon} \frac{G}{\pi} \frac{M_p^2}{R_p^3} = \frac{f}{t \epsilon} \frac{1}{\sqrt{6 \pi G}} v_{esc}^3 \sqrt{\rho_p} \propto v_{esc}^3 \sqrt{\rho_p}.$$
